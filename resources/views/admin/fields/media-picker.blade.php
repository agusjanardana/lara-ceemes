...
<dialog class="ceemes-dialog ceemes-picker-dialog" id="{{ $pickerId }}" data-relation-picker data-input-name="{{ $postedName }}" data-empty-label="Belum ada Media dipilih">
    ...
    <div class="ceemes-picker-toolbar"><label class="ceemes-search-box"><span>Search</span><input type="search" placeholder="Cari nama file, title, alt, atau tipe file..." data-picker-search></label></div>
    <div class="ceemes-media-picker-grid">
        @foreach($mediaItems as $media)
            <label class="ceemes-media-picker-option" data-picker-item data-search="{{ strtolower(($media->title ?: '').' '.$media->original_filename.' '.($media->alt ?: '').' '.$media->mime_type) }}">
